...
test('Test schema generation end to end', async() => {
    ...
    const schemaDoc = await dummyTransformer.processMySQLSchemaOverJDBCWithCredentials(testDBUser, testDBPassword,  testDBHost, testDBName)
    ...
    for(const node of schemaDoc.definitions) {
